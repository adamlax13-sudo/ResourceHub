...
export function AnalyticsLeastClickedWidget({ compact }: { compact?: boolean }) {
  ...
  const displayServices = compact ? reversed.slice(0, 5) : reversed.slice(0, 10);
    ...
    <Card className="bg-white border-gray-100 shadow-sm rounded-xl">
      <CardHeader className="pb-2">
        <CardTitle className="text-base font-medium text-gray-900">
          ...
          <InfoTip text="Active services that rarely get clicked -- may need better categorization or descriptions." />
        </CardTitle>
      </CardHeader>
      <CardContent>
        {isPending ? (
          ...
          <p className="text-sm text-gray-400 text-center py-8">No service data available.</p>
        ) : (
